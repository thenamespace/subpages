      ...
      <Dialog.Portal>
        <Dialog.Overlay
          className={cn(
            ...
            "motion-safe:data-[state=open]:animate-[dialog-fade_200ms_cubic-bezier(0.32,0.72,0,1)]",
          )}
        />
        {/*
          Centred by a full-screen grid, not by translating the panel. The
          panel's enter animation owns its transform; when the centring lived
          on the panel too, the two stacked for the length of the animation
          and the dialog opened up and to the left before snapping into place.

          Clicks on this wrapper still close the dialog: it is outside
          Dialog.Content, so Radix treats them as outside clicks.
        */}
        <div className="fixed inset-0 z-50 grid place-items-center p-4">
          <Dialog.Content
            className={cn(
              // Max height so a long record list scrolls inside the dialog
              // rather than growing it off-screen.
              "w-full max-w-2xl max-h-[calc(100dvh-2rem)] overflow-y-auto",
              "border-edge bg-panel scanlines border-2",
              "shadow-[8px_8px_0_var(--color-ink-950)]",
              "origin-center",
              "motion-safe:data-[state=open]:animate-[dialog-in_200ms_cubic-bezier(0.32,0.72,0,1)]",
              className,
            )}
          >
